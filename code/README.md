...
- `prompts.json` — Seven prompt strings: `normal`, `anon_baseline` (identical to `normal`, separate key for clarity), `explain`, `visual_priority`, `factual_priority`, `q_only`, and `answer_extraction_judge`.
- `example_inference.py` — Runs one CVLAT trial on a single model and extracts a letter answer.
...
| Experiment 2, anonymised-baseline (V_anon) | `anon_baseline` |
